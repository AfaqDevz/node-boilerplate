...
router.post("/register", async (req, res) => {
    try {
        ...
        await sendRes(res, 201, { user }, false, "User registered successfully. Check your email for the temporary password.")
    } catch (error) {
        console.error(error)
...
router.post("/login", async (req, res) => {
    try {
        ...
        jwt.sign(
            ...
            (err, token) => {
                if (err) throw err
                sendRes(res, 200, { token, isFirstLogin: user.isFirstLogin, user }, false, "Login successful")
            }
        )
    } catch (error) {
        console.error(error)
